...
class OBO_Handler
	...
	# => @ancestors_index :: hash of ancestors/descendants per each term handled with any structure relationships
	...
	# CONSTRUCTOR
	#############################################
	def initialize(file, load: false, build_index: false)
...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	def get_familiar(term:, return_ancestors: true)
		...
		return false if @ancestors_index.nil?
		return false if @ancestors_index[term].nil?
		term = term.to_sym if term.is_a? String
		# Find into parentals
		return return_ancestors ? @ancestors_index[term][:ancestors] : @ancestors_index[term][:descendants]		
	end
	...
	def get_IC(term: ,type: :resnick, force: false, zhou_k: 0.5)
		...
		case type
			...
			when :zhou
				...
				ic_seco = 1 - Math.log10(@meta[term][:struct_freq]).fdiv(Math.log10(@stanzas[:terms].length - @alternatives_index.length))
