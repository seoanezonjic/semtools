...
class TestOBOParserFunctionalities < Minitest::Test
	...
	def test_expand
		# Test regular
		assert_equal([:hierarchical,[:Parental]],OBO_Handler.expand_tag(:Child2, @load_Hierarchical[2][:terms], :is_a))
		# Test with already expanded info
		aux_expansion = {Parental: [:A]}
		assert_equal([:hierarchical,[:Parental, :A]],OBO_Handler.expand_tag(:Child2, @load_Hierarchical[2][:terms], :is_a, aux_expansion))		
		# Test with alternatives
		aux_expansion = {Parental: [:A]}
		assert_equal([:hierarchical,[:Parental, :A]],OBO_Handler.expand_tag(:Child2, @hierarchical_terms, :is_a, aux_expansion, " ! ", 0, {B: :Parental}))		
		# Test circular
		assert_equal([:circular,[:C,:B]],OBO_Handler.expand_tag(:A, @load_Circular[2][:terms], :is_a))
		assert_equal([:circular,[:B,:A]],OBO_Handler.expand_tag(:C, @load_Circular[2][:terms], :is_a))
	end
